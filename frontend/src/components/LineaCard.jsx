...
export default function LineaCard({ linea, index = 0 }) {
  return (
    <Link
      ...
      className="surface card-lift stagger-item group flex flex-col overflow-hidden rounded-2xl"
      style={{ animationDelay: `${index * 60}ms` }}
    >
      <div className={`h-28 w-full bg-gradient-to-br ${linea.gradient}`} />
      <div className="flex flex-1 flex-col gap-2 p-6">
        <h3 className="text-lg font-semibold text-white">{linea.nombre}</h3>
        <p className="flex-1 text-sm text-neutral-400">{linea.resumen}</p>
